...
(async()=>{try{
  ...
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',
    {attribution:meta.source||'© OpenStreetMap'}).addTo(map);

  /* Groupes catégories & provenances */
  ...
  for(const p of (provs||[])){
    ...
    if(!ll){console.warn('[Provenance ignorée]',p);continue}
    L.marker(ll,{icon}).addTo(grp).bindPopup(
      `<strong>${p.id? p.id+' – ':''}${p.nom||''}</strong><div>${p.adresse||''}</div><div style="margin-top:6px"><em>${etatToLib[etat]||''}</em></div>`
    );
  }

  /* Statut fusionné & capacité */
  const catToStatut={
  ...
  for(const Lieu of (lieux||[])){
    // Catégorie "effective" : si le texte contient "à vérifier" -> passe en 'limite'
    const raw = norm((Lieu.description||'') + ' ' + (Lieu.analyse||'')); // accepte un champ 'analyse' s'il est ajouté plus tard
    ...
    if(!ll){console.warn('[Lieu ignoré]',Lieu);continue}
    ...
    const nom=(Lieu.nom||'').trim(), titre=`<strong>n°${Lieu.id} — ${nom}</strong>`;
    ...
    // 🔗 Liens cliquables
    const lienAnnonce = Lieu.lien ? a(Lieu.lien, 'Voir l’annonce') : '';
    const lienSite    = Lieu.site ? a(Lieu.site, 'Site du gîte')    : '';
  ...
  function setCollapsed(x){ toolbar.classList.toggle('collapsed',x); tbBtn.setAttribute('aria-expanded',(!x).toString()); tbBtn.textContent=x?'−':'▾'; }
